...
@section('title', 'Update Beasiswa | Dashboard Koordinator')
@section('content')
<section class="w-full bg-white border-2 shadow-sm p-10 rounded-md">
    <h1 class="text-2xl font-semibold">Laporan Beasiswa Anak Asuh</h1>
    <p>Perbarui data</p>
</section>
<section class="grid grid-cols-5 gap-8 mt-10">
    ...
    <div class="col-span-4">
                ...
                <table class="table p-4 bg-white w-full">
                    <tbody>
                        <tr class="text-gray-700">
                            ...
                            <td class="p-4 font-bold">
                                {{ $data->first()->created_at->format('d F Y') }}
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <div class="border-2 bg-white p-4 mt-8">
            <form action="{{ route('koordinator.update-beasiswa', $data->first()->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
