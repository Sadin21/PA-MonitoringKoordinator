...
@section('title', 'Update Wali | Dashboard Koordinator')
@section('content')
<div>
    <section class="w-full bg-white border-2 shadow-sm p-10 rounded-md">
        <h1 class="text-2xl font-semibold">Perbarui Data Wali</h1>
        <form action="{{ route('koordinator.update-data-wali', $data->id) }}" method="POST" class="container bg-white w-full grid grid-cols-2 gap-8 mt-10"  enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div>
